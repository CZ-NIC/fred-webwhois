...
class WebwhoisAssertMixin(object):
    """
    Mixin providing common assertion methods.
    ...
    def assertXpathEqual(self, response, selector, values, transform=transform_to_html, normalize=True):
        """
        Assert that a xpath selector `selector` selects a particular list of `values` values from `response` content.
    ...
    def assertCssSelectEqual(self, response, selector, values, transform=transform_to_html, normalize=True):
        """
        Assert that a css selector `selector` selects a particular list of `values` values from `response` content.
    ...
    def _get_parsed_doctree(self, response):
        """
        Parse HTML document into etree.

        Response must be one of the type:
    ...
    def normalize_spaces(value):
        r"""
        Remove leading and trailing spaces from the string, and replace all internal sequences of white space with one.
